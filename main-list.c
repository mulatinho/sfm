...
{
	GtkWidget *frame, *but;
	GtkWidget *box, *menu, *vbox2, *hbox2, *vbox, *hbox;
	...
	gtk_box_pack_start(GTK_BOX(hbox2), entry1, TRUE, TRUE, 1);
	...
	//gtk_container_add(GTK_CONTAINER(vbox2), clist);
	free(text);

	image = gtk_image_new_from_file("logo.png");
	box = gtk_event_box_new();
	gtk_container_add(GTK_CONTAINER(box), image);
	gtk_box_pack_start(GTK_BOX(vbox2), GTK_WIDGET(box), TRUE, TRUE, 0);

	scrolled = gtk_scrolled_window_new(NULL, NULL);
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scrolled), GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);
	gtk_container_add(GTK_CONTAINER(hbox), scrolled);

	clist_two = gtk_clist_new(1);
	gtk_clist_set_shadow_type(GTK_CLIST(clist_two), GTK_SHADOW_OUT);
	...
	g_signal_connect(GTK_OBJECT(clist_two), "select_row", GTK_SIGNAL_FUNC(do_select), NULL);
	g_signal_connect(G_OBJECT(win), "delete_event", gtk_main_quit, NULL);
	g_signal_connect(GTK_OBJECT(box), "button_press_event", gtk_main_quit, NULL);
